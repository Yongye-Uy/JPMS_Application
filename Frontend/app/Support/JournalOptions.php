...
class JournalOptions
{
    ...
    {
        $cacheKey = 'journal_options';

        if (\Illuminate\Support\Facades\Cache::has($cacheKey)) {
            return \Illuminate\Support\Facades\Cache::get($cacheKey);
        }

        $lock = \Illuminate\Support\Facades\Cache::lock('journal_options_lock', 10);
        
        try {
            $lock->block(5);

            return \Illuminate\Support\Facades\Cache::remember($cacheKey, 600, function () use ($backend) {
